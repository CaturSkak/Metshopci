...
class appcontact2 extends CI_Controller
{
    ...
    {
        $data['ambil'] = $this->customer_model->get_customer_id($pengguna_id);
        $data['user'] = $this->db->get_where(
            'tb_pengguna',
            ['email' => $this->session->userdata('email')]
        )->row_array();
        $this->load->view('admin/header', $data);
        $this->load->view('admin/app-contact-grid2', $data);
        $this->load->view('admin/footer');
    }
    ...
    function updatecustomer()
    {
        ...
        redirect('appcontact2');
    }
    function delete($pengguna_id)
    {
        $this->customer_model->delete($pengguna_id);
        redirect('appcontact2');
    }
}
